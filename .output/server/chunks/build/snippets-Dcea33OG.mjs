...
import { _ as _export_sfc, W as useUserStore, V as VApp, h as VIcon, g as VCard, Q as VImg, bI as VProgressCircular, X as VCardTitle, Y as VCardText, k as VAvatar, e as VSpacer, j as VDivider, a2 as VCardActions, f as VBtn, a0 as VDialog, bt as VSwitch } from './server.mjs';
...
import { V as VAlert } from './VAlert-BpPvQSVy.mjs';
import { V as VToolbar, a as VToolbarTitle } from './VToolbar-Cwh-hMsA.mjs';
import { V as VForm } from './VForm-DigzH7RY.mjs';
import { V as VTextarea } from './VTextarea-Bo6BF8Al.mjs';
import { V as VFileInput } from './VFileInput-DIIM0I_z.mjs';
import '../_/nitro.mjs';
import 'node:http';
import 'node:https';
import 'node:fs';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const filteredWorldSnippets = computed(() => {
      let snippets2 = userStore.worldSnippets ? userStore.worldSnippets.slice(0, 30) : [];
      if (searchQuery.value) {
        const query = searchQuery.value.toLowerCase();
        snippets2 = snippets2.filter(
          (snippet) => {
            var _a, _b;
      ...
      if (frameworkFilter.value !== "All") {
        snippets2 = snippets2.filter((snippet) => snippet.framework === frameworkFilter.value);
      }
      return snippets2;
    });
    const filteredPersonalSnippets = computed(() => {
      let snippets2 = userStore.personalSnippets ? userStore.personalSnippets.slice(0, 30) : [];
      if (searchQuery.value) {
        const query = searchQuery.value.toLowerCase();
        snippets2 = snippets2.filter(
          (snippet) => {
    ...
    const filteredFavoriteSnippets = computed(() => {
      ...
      let snippets2 = [...worldSnippets, ...personalSnippets];
      snippets2.forEach((snippet) => {
    ...
    const sendSnippets = async () => {
      try {
        ...
        snackbarText.value = "Snippet cr\xE9\xE9 avec succ\xE8s";
        snackbarColor.value = "success";
        snackbar.value = true;
      } catch (err) {
        console.error("Error adding snippet:", err.message, err.stack);
        snackbarText.value = "Erreur lors de la cr\xE9ation du snippet";
    ...
    const toggleFavorite = async (snippet, type) => {
      ...
      try {
        if (snippet.isFavorite) {
          ...
          snackbarText.value = "Ajout\xE9 aux favoris";
        } else {
          await userStore.removeFavorite(snippet.id);
          snackbarText.value = "Retir\xE9 des favoris";
        }
        snackbarColor.value = "success";
        snackbar.value = true;
        await userStore.loadSnippets();
      } catch (error) {
        console.error("Erreur lors de la mise \xE0 jour des favoris:", error);
        snippet.isFavorite = !snippet.isFavorite;
        snackbarText.value = "Erreur lors de la mise \xE0 jour des favoris";
        snackbarColor.value = "error";
        snackbar.value = true;
      }
    };
                    ...
                    default: withCtx((_3, _push4, _parent4, _scopeId3) => {
                      if (_push4) {
                        ...
                        _push4(ssrRenderComponent(VCardText, { class: "pt-4" }, {
                          default: withCtx((_4, _push5, _parent5, _scopeId4) => {
                            if (_push5) {
                              _push5(`<p data-v-6fc203b4${_scopeId4}>\xCAtes-vous s\xFBr de vouloir supprimer ce snippet ? Cette action est irr\xE9versible.</p>`);
                            } else {
                              return [
                                createVNode("p", null, "\xCAtes-vous s\xFBr de vouloir supprimer ce snippet ? Cette action est irr\xE9versible.")
                              ];
                            }
                          }),
                          _: 1
...
//# sourceMappingURL=snippets-Dcea33OG.mjs.map
