...
export function createUnauthorizedError(message: string = 'Non autorisé', code?: string | number): ExtendedError {
  return createCustomError(message, 401, code);
}

export function createForbiddenError(message: string = 'Accès interdit', code?: string | number): ExtendedError {
  return createCustomError(message, 403, code);
}

export function createNotFoundError(message: string = 'Ressource non trouvée', code?: string | number): ExtendedError {
  return createCustomError(message, 404, code);
}

export function createServerError(message: string = 'Erreur interne du serveur', code?: string | number): ExtendedError {
