...
async function crawlWebsite(baseUrl: string, maxPages: number = 50): Promise<string[]> {
  ...
  function shouldKeepWithoutVisiting(url: string): boolean {
    try {
      ...
      const excludedExtensions = ['jpg', 'jpeg', 'png', 'gif', 'svg', 'mp4', 'webm', 'pdf', 'doc', 'docx', 'xls', 'xlsx', 'zip', 'rar'];
      if (extension && excludedExtensions.includes(extension)) {
        return true;
      }

      // Exclure les URL avec des paramètres de pagination ou de filtrage
      if (urlObj.search.includes('page=') || urlObj.search.includes('filter=') ||
        urlObj.search.includes('sort=') || urlObj.search.includes('tag=')) {
        return true;
      }

      // Éviter les URL très longues ou avec trop de paramètres
      if (url.length > 150 || urlObj.search.split('&').length > 3) {
        return true;
      }
