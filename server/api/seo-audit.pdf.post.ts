...
import type { CrawlReport } from './seo-audit';

export default defineEventHandler(async (event) => {
  const body = await readBody(event);
  const { url, report } = body as { url: string; report: CrawlReport };
  ...
  const calculateOverallScore = (result: any) => {
    ...
    if (result.coreWebVitals.FCP > 1000) score -= 5;
  ...
  const ensureSpace = (requiredSpace: number, nextSectionTitle?: string) => {
    ...
    if (currentPosition + requiredSpace > doc.page.height - doc.page.margins.bottom) {
      ...
      if (nextSectionTitle) {
        doc.font('Helvetica-Bold')
          .fontSize(18)
          .fillColor('#1976D2')
          .text(nextSectionTitle, { underline: true })
          .moveDown();
        return true;
      }
      return false;
  ...
  const createTable = (data: string[][], options: { headerColor?: string; rowColors?: string[]; columnWidths?: number[] } = {}) => {
    const cellPadding = 12;
    const columnWidths = options.columnWidths || [120, 300, 100];
    const totalWidth = columnWidths.reduce((a, b) => a + b, 0);
    const rowHeight = 50;

    if (doc.y + rowHeight * data.length > doc.page.height - 100) {
      doc.addPage();
    }

    const startY = doc.y;
    doc.fillColor(options.headerColor || '#E3F2FD')
      .rect(50, startY, totalWidth, rowHeight)
      .fill();
    ...
    for (let i = 0; i < data[0].length; i++) {
      ...
      doc.fillColor('#1976D2')
        .font('Helvetica-Bold')
        .text(data[0][i], x + cellPadding, startY + (rowHeight - 20) / 2, {
    ...
    for (let rowIndex = 1; rowIndex < data.length; rowIndex++) {
      ...
      doc.fillColor(options.rowColors?.[rowIndex - 1] || '#FFFFFF')
        .rect(50, rowY, totalWidth, rowHeight)
        .fill();
      ...
      for (let colIndex = 0; colIndex < data[rowIndex].length; colIndex++) {
        ...
        doc.fillColor('#000000')
          .font('Helvetica')
          .text(data[rowIndex][colIndex], x + cellPadding, rowY + (rowHeight - 20) / 2, {
      ...
      doc.strokeColor('#E0E0E0')
        ...
        .lineTo(50 + totalWidth, rowY + rowHeight)
        .stroke();
  ...
  const createBarChart = (data: { label: string; value: number; max: number }[], title: string) => {
    const startY = doc.y;
    ...
    data.forEach((item, index) => {
      ...
      const color = item.value > item.max * 0.8 ? '#F44336' :
        item.value > item.max * 0.6 ? '#FF9800' : '#4CAF50';

      doc.fillColor(color)
        .rect(x, startY + chartHeight - barHeight + 30, barWidth, barHeight)
        .fill();

      doc.font('Helvetica')
        .fontSize(11)
        .fillColor('#000000')
        .text(item.label, x - 50, startY + chartHeight + 40, {
          width: barWidth + 100,
          align: 'center'
        });
      ...
      doc.font('Helvetica')
        .fontSize(11)
        .fillColor('#000000')
        .text(item.value.toString(), x - 50, startY + chartHeight + 60, {
          width: barWidth + 100,
          align: 'center'
        });
    });
  ...
  doc.font('Helvetica')
    .fontSize(14)
    .fillColor('#000000')
    .text(`Analyzed URL: ${url}`, { align: 'left' })
  ...
  const summaryData = [
    ...
    ['Average loading time', `${(report.summary.averageLoadTime / 1000).toFixed(2)}s`],
    ...
    ['Average First Contentful Paint', `${(report.summary.averageFCP / 1000).toFixed(2)}s`],
    ['Average Largest Contentful Paint', `${(report.summary.averageLCP / 1000).toFixed(2)}s`],
    ['Average Time to First Byte', `${(report.summary.averageTTFB / 1000).toFixed(2)}s`],
  ...
  doc.fillColor('#E3F2FD')
    .rect(50, yPosition, columnWidth * 2, 30)
    .fill();

  doc.fillColor('#1976D2')
    .text(summaryData[0][0], 50 + cellPadding, yPosition + cellPadding)
    .text(summaryData[0][1], 50 + columnWidth, yPosition + cellPadding);
  ...
  for (let i = 1; i < summaryData.length; i++) {
    doc.fillColor('#000000')
      .text(summaryData[i][0], 50 + cellPadding, yPosition + cellPadding)
      .text(summaryData[i][1], 50 + columnWidth, yPosition + cellPadding);

    doc.strokeColor('#E0E0E0')
      .moveTo(50, yPosition + 30)
      .lineTo(50 + columnWidth * 2, yPosition + 30)
      .stroke();
  ...
  doc.font('Helvetica')
    .fontSize(12)
    .fillColor('#000000')
    .text('The Core Web Vitals and performance metrics are crucial for SEO because they affect user experience and ranking in search engines.', { align: 'left' })
    .moveDown();

  const createPerformanceMeter = (label: string, value: number, good: number, poor: number, unit: string = 'ms') => {
    const y = doc.y;
    doc.text(`${label}: ${(value / 1000).toFixed(2)}s`, 50, y);
    ...
    doc.strokeColor(color)
      .lineWidth(10)
      .moveTo(50, y + 15)
      .lineTo(50 + barLength, y + 15)
      .stroke();

    doc.moveDown(1.5);
  };

  createPerformanceMeter('First Contentful Paint (FCP)', report.summary.averageFCP, 1000, 3000);
  createPerformanceMeter('Largest Contentful Paint (LCP)', report.summary.averageLCP, 2500, 4000);
  ...
  if (!titleWritten) {
    doc.font('Helvetica-Bold')
      .fontSize(18)
      .fillColor('#1976D2')
      .text('Page Analysis', { underline: true })
      .moveDown();
  }
  ...
  doc.font('Helvetica')
    .fontSize(12)
    .fillColor('#000000')
    .text('Here is the detailed analysis of your site pages, sorted by the number of issues detected. This analysis helps quickly identify pages that need special attention.', { align: 'left' })
    .moveDown();

  const pagesData = Object.entries(report.seoResults)
    .map(([url, result]) => ({
      url,
  ...
  doc.font('Helvetica')
    .fontSize(12)
    .fillColor('#000000')
    .text('Mobile compatibility is a crucial factor for SEO. A well-optimized site for mobile offers a better user experience and is favored by Google.', { align: 'left' })
    .moveDown();
  ...
  doc.font('Helvetica')
    .fontSize(12)
    .fillColor('#000000')
    .text(socialText, 50, doc.y, { align: 'left', width: 500 })
    .moveDown();
  ...
  doc.font('Helvetica')
    .fontSize(12)
    .fillColor('#000000')
    .text(securityText, 50, doc.y, { align: 'left', width: 500 })
    .moveDown();
  ...
  doc.font('Helvetica')
    .fontSize(12)
    .fillColor('#000000')
    .text(recommendationsText, 50, doc.y, { align: 'left', width: 500 })
    .moveDown();
  ...
  createTable(recommendationsData, {
    ...
    rowColors: recommendations.map((_, index) =>
      index < 3 ? '#FFF3E0' :
        index < 6 ? '#E3F2FD' : '#E8F5E9'
    )
  });
  ...
  doc.font('Helvetica')
    .fontSize(12)
    .fillColor('#000000')
    .text('This SEO audit report provides a complete overview of your website. The proposed recommendations aim to improve your visibility in search engines and user experience.', { align: 'left' })
    .moveDown()
    .text('For optimal results, it is recommended to:', { align: 'left' })
    .moveDown(0.5)
    .text('• Implement the top priority recommendations first', { align: 'left' })
    .text('• Regularly monitor your site performance', { align: 'left' })
    .text('• Perform periodic SEO audits to track progress', { align: 'left' })
    .text('• Stay up-to-date with the latest SEO trends and best practices', { align: 'left' })
    .moveDown(2);
  ...
  doc.font('Helvetica')
    .fontSize(10)
    .fillColor('#666666')
    .text('This report was automatically generated by the SEO audit tool.', { align: 'center' })
    .text(`© ${new Date().getFullYear()} StackUnity - All rights reserved`, { align: 'center' });
