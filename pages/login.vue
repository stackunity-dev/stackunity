...
onMounted(async () => {
  try {
    console.log('[LOGIN] Initialisation de la page de connexion');
    const token = TokenUtils.retrieveToken();

    if (token) {
      console.log('[LOGIN] Token trouvé, validation en cours');
      const validation = await userStore.validateToken();
      console.log('[LOGIN] Résultat de la validation:', validation);

      if (!validation.valid) {
        console.log('[LOGIN] Token invalide, suppression');
        TokenUtils.removeToken();
      } else {
        console.log('[LOGIN] Token valide, vérification de l\'authentification');
        await userStore.loadData();
      }
    } else {
      console.log('[LOGIN] Aucun token trouvé');
    }
...
async function handleRedirection() {
  console.log('[LOGIN] Vérification de la redirection, isAuthenticated:', userStore.isAuthenticated);
