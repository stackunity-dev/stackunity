<template>
  <v-app>
    <v-app-bar flat color="primary" elevation="2">
      <v-toolbar-title class="text-h5 font-weight-bold">Code Snippet Editor</v-toolbar-title>
      <v-spacer></v-spacer>
    ...
    <v-main>
      <v-card class="rounded-lg pa-4" v-if="snippetData">
        <v-card-title class="text-h5 font-weight-bold">{{ snippetData.title }}</v-card-title>
        <v-card-subtitle class="pt-2">
          <v-chip size="small" color="primary" class="mr-2">{{ snippetData.framework }}</v-chip>
          <v-chip size="small" variant="outlined" class="mr-2">
            <v-icon start size="small">mdi-calendar</v-icon>
            {{ new Date(snippetData.createdAt).toLocaleDateString() }}
          </v-chip>
        </v-card-subtitle>
      </v-card>
      <client-only>
        <v-container fluid class="pa-4 fill-height">
          <v-row v-if="isLoading" class="fill-height" align="center" justify="center">
            <v-col cols="12" class="text-center">
              ...
              <div class="mt-4 text-h6">Chargement du snippet...</div>
            </v-col>
          </v-row>
          ...
          <v-row v-else-if="!snippetData" class="fill-height" align="center" justify="center">
            <v-col cols="12" class="text-center">
              ...
              <div class="mt-4 text-h6">Snippet non trouvé</div>
